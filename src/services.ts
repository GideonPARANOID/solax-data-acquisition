...
const unmarshallRealTimeData = ({ Data: data, Information: info, ver, type }: RealTimeAPI) => ({
  pv: [
    {
      current: data[0],
      voltage: data[2],
...
export const getRealTimeData = async () => {
  ...
  const { data }: AxiosResponse<RealTimeAPI> = await axios.post(`${config.solaxURL}/?optType=ReadRealTimeData`);
